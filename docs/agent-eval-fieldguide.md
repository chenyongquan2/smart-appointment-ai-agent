...
| 第2层·①数据 ②指标 ③判法 | 184 条用例（dev 154 每类≥30 + held-out 30，含 12 条多轮） / 多指标(含 N/A 设计) / 客观比对 + LLM-judge(改造 4，未校准) | §4、§5、§6 |
...
|---|---|---|
| **规模** | 几条没统计意义，通常要几百~几千条 | ⚠️ **184 条**（[cases.jsonl](../evals/cases.jsonl)：dev 154 条每类≥30 + held-out 30 条；含 12 条多轮，改造 8 数据集切片 + `evals-dataset-scaleup-v2`）——比几百~几千条的门槛仍有距离，但已从「每类勉强够最低统计意义」抬到「每类 30」这一台阶 |
...
| **— 落地（§8）—** | | |
| CI 门禁（基线 + 阈值阻断） | ✅ | `--update-baseline` 落盘基线 + `--gate` 比对回归（退出码 3），接进闸门 2<br>• 只守正确性子集，容差吸收抖动。**现实守 2 项：工具 F1 + 槽位完整率**——「意图」已随 `retire-legacy-intent-classifier`（archive `2026-07-27`）移出，`GATED_METRICS` 3→2<br>• [baseline.json](../evals/baseline.json) 已随数据集切片重定为 **dev 41 条集**（意图 77.2% / 工具 F1 55.9% / 槽位 91.7%，samples=3，人审批准）；重定后 3 次 `--gate` 均 PASS 实守 3 项。门禁恒基于 dev，held-out 不影响退出码 |
...
- **第四切片 ✅（`evals-dataset-scaleup-v2`）**：把规模下限从「每类 ≥5 / held-out 10」抬到**「dev 每类 ≥30 / held-out ≥30 覆盖 5 类」**——全量 51 → **184 条**（dev 154 + held-out 30，多轮 6 → 12，标 `expected_outcome` 29 → 68）。判断力点有两处：① **先盘点再造数**——实测发现 appointment 原有 20 条里 10 条是同一祈使模板（`evals-stabilize-gate-three` 为压方差加的冗余锚点），据此明确本轮**以多样性优先**、不复制该模板，并**提前对齐「指标点估计会下探」的预期**（被替换掉的正是人为压低方差的样本，新增的是它们测不到的难例；验收看「CI 收窄 + 分布合理」而非追旧数字）；② **schema 自检脚本**把「单轮/多轮互斥、split 合法、工具名合法、`expected_outcome` 必在 `expected_tools` 内（防自相矛盾）、输入无重复、held-out 与 dev 无同句（防泄漏）」这些约束变成可执行断言，而不是靠人肉核对。
...
| Q7 | 评估用例从哪来、够不够？ | 理想是生产采样+人工标注，几百上千条，切 held-out | 现 184 条（dev 154 每类≥30 + held-out 30），绝大多数手写合成；已切 dev/held-out 防过拟合，规模距几百~几千条仍有距离但已上一个台阶 | §4 |
...
4. **最硬的短板**：数据集规模（现 dev 每类 ≥30、held-out 30，已比早期每类 ≥5 好一个台阶，但仍未到几百条量级），指标 CI 仍偏宽——规模化属持续投入项。
...
**① 数据平面｜规模化 + 治理**
- 数据集从「手写 184 条」→ **从生产 trace 持续采样 + 标注流水线**（标注工具、多人标注、一致性 κ 系数、黄金集）。
...
| [evals/cases.jsonl](../evals/cases.jsonl) | 184 条评估用例（dev 154 + held-out 30；单轮 `input` + 多轮 `turns`，标期望意图/工具/槽位/split） | §4 |
