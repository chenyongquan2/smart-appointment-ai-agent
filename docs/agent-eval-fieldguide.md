...
**本项目卡在哪层**（诚实点，完整证据见 §10）：已从「只评分类器」升级为「端到端 agent 评估」——组件级意图分类 + 端到端工具调用（参数级/序列级）真评，多轮轨迹⚠️部分（改造 8 多轮切片已归档）；真正还缺的是**系统级业务指标**。能清楚说出「评到哪层、还差哪层」本身就是分层认知的最佳证明（见 §12 陷阱题、§13 话术）。
...
**一句话结论**：**已从「分类器评估」升级为「端到端 agent 评估」并接上回归门禁 + 在线闭环**（改造 1〜4、6、7），且已切出 `dev`/`held-out` 留出集补上「防过拟合」这个数据可信度硬缺口（改造 8 数据集切片）。剩下的缺口集中在**持续投入**：数据集规模化到几百条、系统级业务指标（纯缺）、token/成本、全链路延迟；RAG 评估（改造 5）暂缓待外部服务迁移。详见 §11。
...
**做系统级/业务指标**（§2 分层里唯一还是纯 ❌ 的一层）。理由：现在组件级、端到端轨迹/多轮、回归门禁、在线闭环、dev/held-out 切分都有了，唯独没有「约成了没、用户满不满意」这种业务信号——这是评估金字塔最顶层、也是最能把「评估」跟真实价值挂钩的一块。次选是数据集规模化到几百条（现在每类才≥5，统计意义仍薄）。

> 💬 **面试怎么说**（"你下一步会怎么改进"）：「我会优先补系统级业务指标。因为现在从组件级到端到端轨迹、再到回归门禁和 dev/held-out 防过拟合都有了，唯独没有『约成了没』这种业务信号——这是评估金字塔里最上面还空着的一层。再往后是把数据集从每类几条规模化到几百条，让每个分项指标都有统计意义。」
...
| # | 问题 | 概念答案 | 项目一句话 | 详见 |
|---|---|---|---|---|
| Q1 | 你怎么评估一个 agent 的性能？ | 分三层（组件/轨迹/系统）× 两时机（离线/在线），两条腿要闭环 | 有离线 + 组件级 + 端到端工具调用 + 部分多轮 + 模拟在线闭环；缺系统级业务指标和数据规模化 | §2、§3 |
...
→ 追问「系统级业务指标呢」「RAG 评估呢」「上次加多轮用例后门禁复核完了吗」——这三个都会露馅（都是真缺口，不是谦虚）。

**✅ 这样说，分四句讲**：
1. **两点判断力**：指标完整性（缺数据显式标 N/A、不伪造分母）+ 计算执行解耦（指标是纯函数、可离线确定性单测）。
2. **功能已覆盖**（真数字，不是摆设）：组件级意图分类 + 端到端工具调用（参数级/序列级 P/R/F1）+ 部分多轮轨迹 + 多次采样置信区间 + CI 回归门禁 + 模拟的在线闭环。
3. **诚实还差三块**：系统级业务指标完全没做；RAG 评估主动暂缓（本地实现要被外部服务替换，现在评没长期价值）；LLM-judge 有机制但未校准、不进门禁。
4. **最新鲜的开放项**：最近加多轮用例后基线已重定，但人审批准 + 门禁复核这两步还没收尾。
