...
## 9. 记忆压缩 / Context Compaction（面试高频）

> **一句话**：压缩是把"装不下的旧上下文"有损地总结掉。主流 agent 有**两个流派**，触发口径和适用场景完全不同——**别照搬 Claude Code，先想清楚你压缩是为了解决"窗口装不下"还是"自己的小窗口裁掉了信息"。**

### 9.1 两大流派对比

| 流派 | 代表 | 触发点 | 做法 | 摘要形态 |
|------|------|--------|------|---------|
| **高水位全量压缩** | **Claude Code** auto-compact / `/compact` | 逼近**模型真实窗口**上限（如 200k 的某百分比）才触发 | 把至今**整段对话**一次性总结，用摘要**替换**原文再继续 | 结构化交接（做了什么 / 改了哪些文件 / 当前状态 / 下一步） |
| **滚动缓冲摘要** | LangChain `ConversationSummaryBufferMemory`、多数 chat 框架 | 持续触发：每当回合**滑出滑动窗口** | 把滑出回合**增量并入** running summary（summary-of-summary） | 一段持续累积的摘要 |

两者共性：**摘要都是结构化的**（非自由文本）；都把"近期原文 + 早期摘要"拼起来喂模型。

### 9.2 Anthropic 自己的最新态度：reset > compaction

对**长跑 agent**，官方把 compaction 列为**次选**，首选 **context reset**：清空窗口 + **结构化交接，真实状态靠产出物（文件/DB）传递**（见 [harness-engineering-official-notes.md](./harness-engineering-official-notes.md) §2.1）。原因：压缩有损、会引发"上下文焦虑"（模型快到极限时草草收尾）。

### 9.3 选型的关键判断（面试可直接讲）

1. **先问"为什么压缩"**——是 ① 撞模型窗口天花板（→ Claude Code 高水位全量），还是 ② 自己用了小滑动窗口、想捞回被裁掉的早期信息（→ 滚动缓冲摘要，小阈值触发）。**目的不同，触发口径就不同。**
2. **reset 的"魂"比"形"重要**——agent 的真实状态应活在**结构化存储**（文件 / DB / 槽位 / 偏好表），而非寄望一段有损散文摘要。压缩只该扛"窗口内叙事连贯性"这一件小事。
3. **黄金法则**（§2.4）——"每个组件都编码一个'模型自己做不到'的假设；模型变强就该拆组件。"别把摘要做重：记住关键业务事实是结构化存储的活，不是摘要的活。
4. **漂移风险**——滚动摘要多代叠加会信息衰减；缓解靠**结构化 schema 强约束保留"约束/未完成项"**，或设"覆盖回合上限触发一次全量纠偏"。

### 9.4 落到本项目（add-context-compaction change 的决策依据）

- 预约/咨询会话**短**，撞不到任何模型窗口 → Claude Code 那套**永不触发**，照搬等于没做。
- 本项目压缩是为 ②：`ShortTermMemory` 故意只留 10 轮，压缩用来**捞回被这个小窗口裁掉的早期信息** → 选**滚动增量 + 小的固定阈值触发**。
- 关键偏好（性别/时间/项目）和槽位状态 → 归 `LongTermMemory` + 结构化 slot（DB），**不靠对话摘要兜底**（这就是 reset 之魂的落地）。
- 全量纠偏（C 方案）：**留开关、默认关**——本业务到不了需要纠偏的会话长度。

### 9.5 一个值得讲的坑：压缩的「夹缝盲区」与修复（fix-compaction-gap-blindspot）

滑动窗口 + 攒批压缩天然会留一个**夹缝**：回合掉出窗口后、到下次压缩前，它「既不在窗内原文、又没进摘要」——这段时间对 LLM **暂时不可见**。失效例：用户 11 轮前说「周六 14:30」，13 轮刚滑出窗口、还没触发压缩，此时说「提前一小时」→ 模型手里既无原文也无摘要，可能答错。

- **本质**：盲区大小 = 自上次压缩以来新增的消息数（峰值 ≈ `min_old_turns`），有界但真实，且与「压缩是为了不丢窗外信息」的初衷相悖。
- **修复（面试可讲的设计判断）**：把读侧**可见性分界从「窗口」改成 `covered_upto`**——id ≤ covered_upto 以摘要注入、id > covered_upto **一律原文注入**。于是「没进摘要的必有原文」，盲区数学上不存在；`window_turns` 退化为只管「写侧何时压缩（节奏）」，不再是「读侧能看几条」。
- **代价**：读侧多带夹缝那几条 token（有界 ≈ window+min_old）+ 一次按主键的 SQLite 读；远小于「答错」的代价。
- **通用启发**：滑动窗口 + 异步/攒批压缩的系统，都要检查「窗口边界」与「压缩游标」之间这条缝——可见性应以**压缩游标**而非窗口为准。
...
| **Compaction（压缩）** | 把装不下的旧上下文有损总结掉；两流派：高水位全量（Claude Code）/ 滚动缓冲摘要（LangChain）|
| **Context Reset** | 清空窗口 + 结构化交接，真实状态靠产出物（文件/DB）传递；长跑 agent 优于 compaction |
| **Summary-of-summary** | 滚动增量压缩：前序摘要 + 新出窗回合 → 新摘要，避免每次全量重算 |
