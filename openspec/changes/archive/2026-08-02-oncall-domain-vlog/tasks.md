...
- [x] 5.3 ✅ **真实环境冒烟已通过**（2026-08-03，连 VPN 后）。实测：三个租户并发探查，uat 命中 571 万、dev 命中 383 万，各返回 3 条完整日志（含真实 `_msg`：MT5RpsHandle.cpp:3842、Druid 连接池报错等），vmui 链接指向真实 host。凭据从 `.env` 加载、错误串无泄漏、env→租户映射（prod=3/uat=2/dev=0）经另一项目的 `VICTORIALOGS_ACCOUNT_ID=3` 交叉印证。

  ⚠ **同时发现一个真缺陷**（离线测试不可能发现，见第 8 组）：prod 宽窗查询会把"太重没算完"报成"0 命中"。
...
## 8. 真实环境冒烟发现的缺陷（2026-08-03）

- [x] 8.1 ★ **prod 宽窗查询的"0 命中"可能是假的**。实测：prod 6h 窗查 `"ERROR"` 返回 0，而同样的词 **30m 窗有 188 条**；`*` 查询显示 prod 6h 内约 **50 亿行**。多数情况下 6h 会走超时（60s）被正确分类，但**观测到过一次返回 200 + 空结果、不报错**——那时模型会自信地告诉用户「prod 没有这个错误」。这正是本项目一路在防的那类「用没查到冒充没有」。
- [x] 8.2 **修复**：`EnvResult` 记录 `elapsed_s`，新增 `zero_suspect` —— 0 命中且耗时超过超时上限一半时，结果里加 `zero_hits_unreliable` 字段、并把提示提到顶层 `hint`。判据来自实测（真 0 是 1.9s 秒回、可疑的 0 耗光 60s）。工具 description 同步加警告。补 4 条回归测试。
- [x] 8.3 ⚠ **如实界定覆盖范围**：验证时试过用**响应形状**区分，结果发现**真 0 与可疑的 0 返回的 JSON 完全一样**（都是 `{"hits": []}`）——VL 就是这么表示零结果的。故若 VL 在重查询下**快速**返回空结果，service 层无从分辨。那种情形只能靠 description 里的约束兜底：**模型不得凭一次宽窗查询断定「没有日志」，必须先收窄窗口重查**。这个限制写进了 `zero_suspect` 的 docstring，不藏着。
