...
> **以下两节于 2026-08-03 从分支 `feat/evals-dataset-scaleup` 摘回**（该分支随后删除）。
> 归档时主干这份 design.md 还是提案期的 35 行 stub，实测回填只在分支上——差点随分支一起丢掉。
> 内容与 change 的放弃状态无关：**它说的是主干现有评分口径的性质**，第 4 期建 oncall 数据集时直接适用。
