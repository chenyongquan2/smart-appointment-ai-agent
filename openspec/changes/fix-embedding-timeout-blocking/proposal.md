...
2. **同步调用跑在事件循环上**：`embed_query` 是同步阻塞调用，却被 `KnowledgeService` 的 **5 处 `async` 方法**直接调用。一旦 HTTP 请求挂住，就占住整个事件循环——连 `asyncio.wait_for` 的定时器回调都跑不了，所以任何外层超时都救不了它。
...
- **改造调用点**：`KnowledgeService` 的 5 处 `async` 方法改为 `await aembed_input(...)`。同步版 `embed_input` **保留**，供 `find_best_match_indices` 这类同步调用点使用（它们不在事件循环的关键路径上）。
...
- **代码**：`config/model_provider.py`（`create_embedding_model` 加超时）、`services/text_embedding.py`（`timeout` 真正生效 + 新增 `aembed_input`）、`services/knowledge_service.py`（5 处调用点改 await 异步版）。
