# test-safety-net Specification

## Purpose

定义重构 `agents/` → `harness/` 前的测试黄金样本与回归防线:测试套件必须在离线、无 API key 条件下确定性通过,async 测试被真正执行,classification 测试不依赖真实 LLM,phantom 测试以 `xfail` 隔离而非误判为回归。
