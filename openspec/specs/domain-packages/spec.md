...
4. **权限策略**——该域的危险操作判定策略；
5. **评估数据与标注口径**——该域的用例集与基线所在目录（`evals_dir`），**以及**一份 `EvalProfile` 声明，含该域的数据集标签集合、槽位键映射、门禁指标集三项。

第 5 项之所以不止是一个目录：评估机制要读用例，就必须知道「本域的标签叫什么、哪些工具入参算槽位、本域实际能守哪些指标」。这三件事**换个域就不成立**，按判据它们属域绑定内容；把它们留在 `evals/` 会让「机制域无关」名不副实——装上另一个域的数据要么直接加载失败，要么门禁静默退化成守更少的项。
...
- **WHEN** 装载任一领域包
- **THEN** 得到的领域对象含工具集、子 Agent 集、系统提示、权限策略、评估数据目录与 `EvalProfile` 五项，缺一即为装载失败

#### Scenario: 域无关机制不随域走

- **WHEN** 检查 `harness/` 与 `evals/`
- **THEN** 其中不存在任何具体领域的工具定义、子 Agent 定义、人设文本、标签名、槽位名或门禁指标组合；`ToolRegistry`、`SubAgentRegistry`、系统提示拼接函数、评估运行器、指标算法均保持域无关

#### Scenario: EvalProfile 为纯声明

- **WHEN** 检视 `EvalProfile` 的定义
- **THEN** 它只承载不可变数据（标签集合、槽位键映射、门禁指标集），不含任何指标计算或门禁比对逻辑
