# task-executor Specification

## Purpose

定义 Channel 与 Agent 之间的任务执行层：以统一的任务接口承接来自各通道（IM 异步、Web 同步）的请求，负责并发记账（同话题串行、跨话题并行、并发与排队上限）、墙钟超时兜底、工具调用超时，并把 Agent 的输出择出为结构化终态。Channel 由此无需解析 Agent 的输出协议，也无需各自实现并发与超时策略。
