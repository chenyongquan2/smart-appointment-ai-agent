...
工具层 SHALL 提供三个专用子 Agent，各持有其领域工具子集：`appointment` 子 Agent 持有 `find_technician` / `check_availability` / `create_appointment` / `get_user_preferences`；`consultant` 子 Agent 持有 `search_knowledge`；`user_behavior` 子 Agent 持有 `get_user_preferences`。各子 Agent 的工具子集 MUST 从既有工具中切片得到，不新建重复工具、不重写其业务逻辑。
