...
class AppointmentAgent:
    ...
    async def run_stream(self, user_input=None):
        ...
        # 1. 结构化抽取用户输入（不向用户流式输出，避免英文字段名暴露在聊天界面）
        slots = await self.input_parser.extract(user_input, self.chat_history)

        try:
            # 2. 转为 dict 视图，兼容下游 data.get(...) 用法
            data = self.input_parser.parse_data(slots)
