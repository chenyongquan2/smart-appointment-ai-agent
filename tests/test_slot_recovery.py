...
def test_merges_slots_across_tools():
    ...
    assert slots_from_tool_calls(tool_calls) == {
        "project": "推拿",
        "gender": "male",
...
def test_same_slot_conflict_last_write_wins():
    ...
    assert slots_from_tool_calls(tool_calls)["project"] == "推拿"
...
def test_sentinel_defaults_not_counted():
    ...
    slots = slots_from_tool_calls(tool_calls)
...
def test_empty_or_none_returns_none():
    ...
    assert slots_from_tool_calls(None) is None
    assert slots_from_tool_calls([]) is None
