...
```bash
AGENT_DOMAIN=oncall uv run python evals/run_evals.py --gate
```

容差**不用手写**——`0.10` 由本域的 `EvalProfile.tolerance` 声明，不传 `--tolerance` 即用它（change `gate-tolerance-per-domain`）。显式传入仍然覆盖，供排障与临时放宽。门禁报告会标明本次容差的来源（域声明 / 命令行覆盖）。
...
**不再向预约域评测投入新精力**：不扩用例、不重定基线、不恢复那份 184 条扩容版（17/18 完成、卡在重定基线——已放弃，change 于 2026-08-03 close 进 `openspec/changes/archive/2026-08-03-evals-dataset-scaleup-v2-abandoned/`，分支 `feat/evals-dataset-scaleup` 同日删除、对象存于 tag `abandoned/evals-dataset-scaleup-v2`；它改的 `evals/cases.jsonl` 现在的路径已是 `domains/appointment/evals/cases.jsonl`，结构上也合不回来了）。
...
- **门禁只守正确性子集，守哪几项由领域包声明**（`EvalProfile.gated_metrics`，change `oncall-evals-bootstrap`）：预约域守 `{工具调用-F1, 槽位抽取完整率}`，值守域守 `{工具调用-F1, 工具调用-参数级F1}`。声明里的名字拼错、或声明了下面这些不准守的指标，**装载即报错退出 2**——静默跳过一个拼错的名字等于门禁少守一项而没人知道。任何域都**刻意排除**：
  ...
  其余指标仍照常打印，只是不触发非零退出。
- **容差**：比率即百分点（30pp），吸收 LLM 的 run-to-run 抖动。比率回归判定为 `当前 < 基线 − 容差`。

  **容差随域声明**（change `gate-tolerance-per-domain`）：取值来自 `Domain.eval_profile.tolerance`，`evals/` 侧**不再保留任何按具体域校准的默认值**——`--tolerance` 不传即用域声明值，显式传入则覆盖。

  | 域 | 容差 | 实测最差半宽（定基线时） |
  |---|---|---|
  | appointment | `0.30` | 槽位完整率 ±28.7pp |
  | oncall | `0.10` | 参数级 F1 ±1.1pp |

  ⚠️ **跨域容差 MUST NOT 互相沿用**——一个按 A 域最差半宽定的阈值用在 B 域上，会静默地守得过松或过紧。这与 `gated_metrics` 是同一类问题：按 A 域校准的全局常量装上 B 域时不报错、只是失准。

  **预约域 0.30 的依据（实测校准、非魔数）**：每次重定基线时观测被守指标的 95% t-CI 半宽（改造 3），容差取值须覆盖最差半宽。change `retire-legacy-intent-classifier` 重定时实测（41 条 dev × 3，干净跑）：**工具 F1 ±5.7pp（历史水位）、槽位抽取完整率 ±28.7pp**。槽位半宽超出原 `0.20`，故按实测上调至 `0.30`。
